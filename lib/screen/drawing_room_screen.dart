...
class _DrawingRoomScreenState extends State<DrawingRoomScreen> {
  ...
  Future<void> _initializeDrawingPoints() async {
    ...
    setState(() {
      ...
      historyDrawingPoints = List.of(points);
    });
  }

  void _listenForUpdates() {
    _firebaseService.listenForUpdates((updatedPoints) {
      setState(() {
        drawingPoints = updatedPoints;
        historyDrawingPoints = List.of(updatedPoints);
      });
    });
  }

  Future<void> _saveCurrentDrawingPoint(DrawingPoint point) async {
    await _firebaseService.saveDrawingPoint(point);
  }
          ...
          GestureDetector(
            onPanStart: (details) {
              setState(() {
                ...
                _saveCurrentDrawingPoint(currentDrawingPoint!);
              });
            },
            onPanUpdate: (details) {
              setState(() {
